...
function bufferToSign(key: string, serializedMessage: Buffer | Uint8Array): Buffer {
...
export class MessageToken {
    ...
    constructor(key: string, message: Message | VersionedMessage, keypair: Keypair) {
        this.key = key;
    ...
    static isValid(key: string, message: Message | VersionedMessage, token: string, publicKey: PublicKey): boolean {
        if (!token) {
            return false;
        }
