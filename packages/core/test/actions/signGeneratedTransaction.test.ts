...
import { Keypair, Connection, Transaction, sendAndConfirmRawTransaction, SystemProgram } from '@solana/web3.js';
...
if (process.env.TEST_LIVE) {
    describe('signGeneratedTransaction action', async () => {
        it('signs a transaction with correct message token', async () => {
            ...
            const messageToken = new MessageToken('test-transaction', transaction.compileMessage(), feePayer).compile();
        ...
        it('rejects a transaction with additional instruction', async () => {
            ...
            transaction.partialSign(user);
            await expect(
                signGeneratedTransaction(connection, transaction, feePayer, 'test-transaction', messageToken, cache)
        ...
        it('rejects a duplicate transaction', async () => {
            ...
            await signGeneratedTransaction(connection, transaction, feePayer, 'test-transaction', messageToken, cache);
            await expect(
                signGeneratedTransaction(connection, transaction, feePayer, 'test-transaction', messageToken, cache)
