...
program
    ...
    .option('-t, --tokens-from-top <number>', 'Tokens from the top of Jupiter aggregator to include', '10')
    .option(
        '-m, --margin <number>',
    ...
    .action(async ({ tokensFromTop, margin, includeAccountFees }) => {
        ...
        const tokensWithPriceInfo = await Promise.all(
            popularTokenMints.map(async (mint) => ({
                mint: mint,
...
program
    ...
    .action(async (mintAsString, { margin, includeAccountFees }) => {
        ...
        const tokenFee = (
            await PayerUtils.buildTokenFeeList(
                connection,
                ENV_SECRET_KEYPAIR.publicKey,
                [{ mint, priceInfo }],
                pricingParams
...
program
    ...
    .option('-d, --dry-run', 'Outputs accounts to be created', '')
...
program
    ...
    .option('-d, --dry-run', 'Outputs swap transactions to be executed', '')
    .option('-t, --threshold <number>', 'Minimum value of tokens to exchange, in SOL lamports', '100000000')
